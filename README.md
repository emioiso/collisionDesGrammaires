## はじめに（注記）

このリポジトリに記している内容は、  学術的な定義や一般論ではなく、私個人の経験と観測にもとづく整理である。
ここで用いている比喩や考え方は、  自身の言語感覚や日常の違和感を理解するための仮説であり、  常に正しいとは限らず、誤りや偏りを含んでいる可能性がある。
そのため、これらは固定された結論ではない。  新しい経験や指摘によって、今後アップデートされる前提の記述である。

内容に不正確な点や、よりよい表現、別の視点があると感じた場合には、ぜひ指摘してほしい。

ただしその際は、誰かを否定するためではなく、対話を前に進めるための指摘として、紳士的に、あるいは淑女的に伝えてもらえるとありがたい。
私はこの記録を、正しさを競うためではなく、違いを理解し合うための途中経過として残している。

---

## 文法の衝突とは

同じ出来事を、複数の文法で並べて記述したとき、意味は同じはずなのに、成立の仕方が一致しない瞬間が生まれる。
このリポジトリは、そのズレそのものを記録し、可視化するための展示である。

---

### 例：「起きた」

**日本語** 
起きた気がする  （曖昧さが許される）

**フランス語**  
Je me suis levée  （主体と完了を要求される）

**C言語**
```c
bool woke_up = true;
//真か偽かを強制される
```
//点字
//⠄⠕⠃⠊ ⠀⠥ ⠀⠊⠅⠕⠞

//点字は、触覚を前提に成立する文法である。指で辿らなければ意味が立ち上がらない、画面上に置かれた点字は、読むための条件を満たしていない。 

//（即時的な理解ができない）

意味は同じはずなのにそれぞれの文法では、同時に成立しないこのズレが可視化される状態を私は「衝突」と呼ぶ。

## 私と文法について
日本語は私の母国語である。曖昧さや余白を含んだままでも、経験として成立する言語だ。
フランス語は、短大で仏文科を卒業し学んできた言語である。性・数・一致といった厳密な構造に、私は安心感を覚える。 少しずつでいいから、自分に起こった出来事くらいは、フランス語として正確に記述したいと思っている。

私はiOSエンジニアであり、普段はSwiftで文法を書いている。一方で、C言語は便利な*関数がほとんど用意されていない。 その不便さは、補助が少ない状態で世界と向き合う感覚に近く、私はこの言語に強い親近感を覚えている。
現在普及している多くの言語がC言語を始祖としていると知り、私はこの文法を、より深く使いこなせるようになりたいと考えている。

*関数とは、「お茶を入れる」のように、細かい手順を一つの名前にまとめたものだ。便利な言語では「お茶を入れる」と言えば済むが、 C言語では「湯を沸かし、茶葉を入れ、注ぐ」と一つずつ書く必要がある。

## 視点の転換としての文法

2022年、30歳のときにADHDとASDの診断を受けた。それをきっかけに、自分が多数派の文法ではなく、少数派の文法で世界を経験してきたことを自覚するようになった。
これまで言語化できなかった違和感は、欠落ではなく、別の文法として整理され始めている。

## 依存という比喩について
私の感覚として、視力を失うことは、視覚への依存があった。点字しか読めないことは、触覚への依存である。
私にとって発達障害は、世界がどのように振る舞うかを予測できる構造への依存である。これは一般化ではなく、あくまで私自身の経験にもとづく理解である。

## 点字について
点字に興味を持ち始めたのは、2025年に障害者手帳の等級が2級となり、自分の身体や感覚について考える時間が増えたことがきっかけである。
人は生きていれば、いずれ緑内障か白内障のどちらかになり、ある日、視力を失う可能性があることを知った。
生きながらに視力を失うことは、どれほどつらいのか。ある物語の一節に背中を押され、私は点字を学ぶことを決めた。
点字は、読むために身体を必要とする文法である。意味は、触れる時間の中で遅れて立ち上がる。2026年現在は点字を勉強中であり、画面上で記述する方法を思案中。

## 最後にこのリポジトリについて
このリポジトリでは、同じ一日、同じ出来事を、日本語・フランス語・C言語で記述する。それらは翻訳されない。 一致しないまま、並べられる。書けない日、空白の日、エラーが残る日も含めて、それらすべてが**「日常が同時に成立しなかった記録」**である。

ここに書かれている内容は、私の理解の途中段階によるものである。
無知や見落としを含んでいる可能性があることを自覚しており、新たな学びや指摘によって、必要だと判断した場合は更新していく。

大礒 恵美

## Introduction (Note)

The content recorded in this repository is not based on academic definitions or general theory, but on my personal experience and observations.
The metaphors and ideas used here are hypotheses intended to help me understand my own linguistic sensibilities and everyday discomforts. They are not always correct and may contain errors or biases.

Therefore, these statements are not fixed conclusions. They are written with the assumption that they may be updated through new experiences or insights.
If you find inaccuracies, better expressions, or alternative perspectives, I welcome your comments.
However, I would appreciate it if such feedback were offered not to negate anyone, but to move the dialogue forward — conveyed in a gentlemanly or ladylike manner.
I leave these records not to compete over correctness, but as an ongoing process toward understanding differences.

## What Is the Exhibition of Grammatical Collision?

When the same event is described through multiple grammatical systems, there are moments when the meaning seems identical, yet the conditions of its formation do not align.
This repository is an attempt to document and visualize that very misalignment.

Example: “I woke up.”

Japanese
起きた気がする
(ambiguity is permitted)

French
Je me suis levée
(the subject and completion are required)

C language
```
bool woke_up = true;
// forces a true/false state
```

// Braille
// ⠄⠕⠃⠊ ⠀⠥ ⠀⠊⠅⠕⠞

// Braille is a grammar that presupposes touch.
// Meaning does not arise unless traced by the fingers.
// Braille displayed on a screen does not fulfill the conditions required for reading.
// (immediate comprehension is not possible)

Although the meaning appears identical, each grammar does not allow it to exist in the same way.
I call the state in which this discrepancy becomes visible “collision.”

## About Me and Grammar
Japanese is my native language.
It allows experience to exist even while containing ambiguity and open space.

French is a language I studied in junior college, majoring in French literature.
In its strict structures — gender, number, agreement — I find a sense of reassurance.
Little by little, I hope to describe at least the events that happen to me with precision in French.

I am an iOS engineer and usually write in Swift.
In contrast, C provides very few convenient built-in functions.
Its inconvenience resembles confronting the world with minimal assistance, and I feel a strong affinity with it.

Knowing that many modern languages trace their origins to C, I wish to deepen my command of its grammar.

In programming, a “function” is like saying “make tea” — a set of small steps gathered under one name. In more convenient languages, one can simply say “make tea.” In C, however, one must write each step: boil the water, add the tea leaves, pour it.

## Grammar as a Shift in Perspective
In 2022, at the age of 30, I was diagnosed with ADHD and ASD.
Through this, I came to recognize that I had been experiencing the world not through the grammar of the majority, but through that of a minority.

What had once felt like inarticulable discomfort began to be reorganized — not as deficiency, but as another grammar.

On the Metaphor of Dependence

From my perspective, losing eyesight reveals a prior dependence on vision.
Being able to read only Braille would be a dependence on touch.

For me, developmental disability can be understood as a dependence on structures that allow prediction of how the world behaves.
This is not a generalization, but an interpretation grounded in my own experience.

## About Braille
My interest in Braille began in 2025, when my disability certificate was classified as Grade 2.
It was a time when I began thinking more deeply about my body and sensory perception.

I learned that, over the course of a lifetime, one may eventually develop glaucoma or cataracts, and that vision may be lost.
What does it mean to lose sight while still alive?

Moved by a line from a story, I decided to study Braille.

Braille is a grammar that requires the body in order to be read.
Meaning emerges with delay, through the time of touch.
As of 2026, I am studying Braille and considering how it might be written or represented on a screen.

## Finally, About This Repository

In this repository, the same day and the same events are written in Japanese, French, and C.
They are not translated.
They are placed side by side without reconciliation.

Days when nothing can be written, blank days, and days containing errors are all included.
Together, they form a record of moments when daily life did not come into being simultaneously.

The content written here reflects my current stage of understanding.
I recognize that it may contain ignorance or oversight.
If I determine that revision is necessary through further learning or thoughtful feedback, I will update it accordingly.
EMI OISO


## Introduction (Note)

Le contenu de ce dépôt ne repose ni sur des définitions académiques ni sur des théories générales, mais sur mes expériences et observations personnelles.
Les métaphores et les idées employées ici sont des hypothèses destinées à m’aider à comprendre ma propre sensibilité linguistique et les inconforts du quotidien. Elles ne sont pas toujours justes et peuvent contenir des erreurs ou des biais.

Par conséquent, ces textes ne constituent pas des conclusions définitives.
Ils sont écrits dans la perspective d’être mis à jour au fil de nouvelles expériences ou remarques.

Si vous percevez des inexactitudes, de meilleures formulations ou d’autres points de vue possibles, je vous invite à me les signaler.

Cependant, je vous serais reconnaissante de formuler ces remarques non pour nier quiconque, mais pour faire avancer le dialogue — avec courtoisie.

Je conserve ces traces non pour rivaliser sur la justesse, mais comme un processus en cours visant à comprendre les différences.

## Qu’est-ce que l’exposition de la collision des grammaires ?

Lorsque le même événement est décrit à travers plusieurs systèmes grammaticaux, il arrive que le sens semble identique, alors que les conditions de son existence ne coïncident pas.

Ce dépôt tente de documenter et de rendre visible ce décalage.

Exemple : « Je me suis levée »

Japonais
起きた気がする
(l’ambiguïté est permise)

Français
Je me suis levée
(le sujet et l’accomplissement sont requis)

Langage C
```
bool woke_up = true;
// impose un état vrai ou faux
```

// Braille
// ⠄⠕⠃⠊ ⠀⠥ ⠀⠊⠅⠕⠞

// Le braille est une grammaire qui présuppose le toucher.
// Le sens n’émerge que lorsqu’il est parcouru par les doigts.
// Un braille affiché à l’écran ne remplit pas les conditions nécessaires à sa lecture.
// (la compréhension immédiate n’est pas possible)

Bien que le sens paraisse identique, chaque grammaire ne lui permet pas d’exister de la même manière.
J’appelle « collision » l’état dans lequel ce décalage devient visible.

## Moi et les grammaires

Le japonais est ma langue maternelle.
Il permet à l’expérience d’exister tout en conservant de l’ambiguïté et des zones ouvertes.

Le français est une langue que j’ai étudiée à l’université, en littérature française.
Dans ses structures strictes — genre, nombre, accords — je trouve un sentiment de stabilité.
Peu à peu, j’aimerais décrire avec précision, en français, au moins les événements qui me concernent.

Je suis ingénieure iOS et j’écris habituellement en Swift.
En revanche, le langage C offre très peu de fonctions pratiques.
Son austérité ressemble à une confrontation avec le monde sans assistance, et j’y ressens une forte affinité.

Sachant que de nombreux langages modernes trouvent leur origine dans le C, je souhaite approfondir ma maîtrise de sa grammaire.

En programmation, une « fonction » est comparable à l’expression « préparer du thé » : un ensemble d’étapes rassemblées sous un seul nom. Dans des langages plus pratiques, il suffit d’écrire « préparer du thé ». En C, il faut décrire chaque étape : faire bouillir l’eau, ajouter les feuilles, verser.

## La grammaire comme déplacement du regard

En 2022, à l’âge de 30 ans, j’ai reçu un diagnostic de TDAH et de TSA.
Cela m’a permis de comprendre que j’avais expérimenté le monde non selon la grammaire de la majorité, mais selon celle d’une minorité.

Les sensations de décalage que je ne parvenais pas à formuler ont commencé à s’organiser non comme un manque, mais comme une autre grammaire.

## À propos de la métaphore de la dépendance

De mon point de vue, perdre la vue révèle une dépendance préalable à la vision.
Ne pouvoir lire que le braille serait une dépendance au toucher.

Pour moi, le handicap développemental peut être compris comme une dépendance à des structures permettant de prévoir le comportement du monde.
Il ne s’agit pas d’une généralisation, mais d’une interprétation issue de mon expérience personnelle.

## À propos du braille

Mon intérêt pour le braille a commencé en 2025, lorsque mon certificat de handicap a été classé en catégorie 2.
C’était un moment où je réfléchissais davantage à mon corps et à mes perceptions.

J’ai appris qu’au cours d’une vie, il est possible de développer un glaucome ou une cataracte, et de perdre la vue.
Que signifie perdre la vue tout en restant en vie ?

Touchée par une phrase lue dans un récit, j’ai décidé d’apprendre le braille.

Le braille est une grammaire qui exige le corps pour être lue.
Le sens apparaît avec retard, dans le temps du toucher.
En 2026, je suis encore en train d’apprendre le braille et de réfléchir à la manière dont il peut être représenté à l’écran.

## Enfin, à propos de ce dépôt

Dans ce dépôt, une même journée et un même événement sont décrits en japonais, en français et en C.
Ils ne sont pas traduits.
Ils sont simplement placés côte à côte, sans être réconciliés.

Les jours où rien ne peut être écrit, les jours vides et les jours comportant des erreurs sont également inclus.
L’ensemble constitue l’archive de moments où le quotidien ne s’est pas constitué simultanément.

Ce qui est écrit ici correspond à l’état actuel de ma compréhension.
Je suis consciente qu’il peut contenir des ignorances ou des oublis.
Si de nouveaux apprentissages ou des remarques pertinentes me conduisent à juger qu’une révision est nécessaire, je procéderai à une mise à jour.

ÉMI OISO

⠕⠕⠊⠎⠕　⠑⠍⠊
